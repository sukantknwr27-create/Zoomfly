...
<div class="zf-footer-wrap">
  ...
  <div class="zf-footer-grid">
    ...
    <div class="zf-footer-brand">
      <a href="/" class="logo" style="margin-bottom:14px;display:inline-flex">
        ${ZF.logoUrl ? `<img src="${esc(ZF.logoUrl)}" alt="${esc(ZF.companyName||'ZoomFly')}" style="height:28px;width:auto;display:block">` : `
        <span class="logo-zoom">Zoom</span><span class="logo-fly">Fly</span>
    ...
    <div class="zf-footer-col">
      ...
      <div style="display:flex;flex-direction:column;gap:12px;">
        <div>
          <a href="tel:${esc(ZF.phone.replace(/\s+/g,''))}" style="display:flex;align-items:center;gap:7px;color:rgba(255,255,255,0.7);font-size:.82rem;text-decoration:none;margin-bottom:3px;">
            <span class="emoji-icon" style="font-size:12px;line-height:1;display:inline-block;vertical-align:middle">📞</span> ${esc(ZF.phone)}
          </a>
          <div style="font-size:.72rem;color:rgba(255,255,255,0.35);padding-left:22px;">Mon – Sat (9 AM – 8 PM)</div>
        </div>
        <div>
          <a href="mailto:${esc(ZF.email)}" style="display:flex;align-items:center;gap:7px;color:rgba(255,255,255,0.7);font-size:.82rem;text-decoration:none;margin-bottom:3px;">
            <span class="emoji-icon" style="font-size:12px;line-height:1;display:inline-block;vertical-align:middle">📧</span> ${esc(ZF.email)}
          </a>
        ...
        <div style="display:flex;align-items:flex-start;gap:7px;font-size:.79rem;color:rgba(255,255,255,0.55);line-height:1.6;">
          ...
          <span>${esc(ZF.address)}</span>
        </div>
        ${ZF.gstin ? `<div style="font-size:.72rem;color:rgba(255,255,255,0.35);">GSTIN: ${esc(ZF.gstin)}</div>` : ''}
        <div>
          <a href="https://wa.me/${esc(ZF.whatsapp)}" style="display:flex;align-items:center;gap:7px;color:rgba(255,255,255,0.7);font-size:.82rem;text-decoration:none;">
  ...
  <div style="padding:16px 28px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;">
    <div style="display:flex;align-items:center;gap:6px;">
      <span style="font-size:.78rem;color:rgba(255,255,255,0.28);">${yr} ${esc(ZF.companyName || 'ZoomFly')}. All rights reserved.</span>
...
function renderWA() {
  document.body.insertAdjacentHTML('beforeend',
    `<a href="https://wa.me/${esc(ZF.whatsapp)}?text=Hi%20ZoomFly!%20I%20want%20to%20enquire%20about%20a%20trip."
...
function tourCard(p, delay = 0) {
  ...
  return `
    <a href="/pages/package-detail.html?id=${esc(id)}"
