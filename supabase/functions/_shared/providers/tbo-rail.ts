// ============================================================
// Provider stub: tbo-rail.ts
// File: supabase/functions/_shared/providers/tbo-rail.ts
//
// STATUS: STUB. Tbo's real rail/IRCTC B2B API is not
// implemented yet — this project has no Tbo rail
// credentials or API documentation on file as of this commit.
//
// Activating "Tbo" for the rail service in the admin
...
// booking. Only checkPnrStatus() (if Tbo offers a real
...
export class TboRailProvider implements RailProvider {
  readonly name = 'tbo';
  ...
  async search(_params: RailSearchParams): Promise<RailTrainResult[]> {
    throw new Error(
      'Tbo rail provider is not implemented yet. ' +
      'See supabase/functions/_shared/providers/tbo-rail.ts.'
    );
  }

  async checkPnrStatus(_pnr: string): Promise<{ status: string; details: string }> {
    throw new Error('Tbo rail provider does not implement PNR status yet.');
  }
}
