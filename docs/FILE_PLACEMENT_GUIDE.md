> ⚠️ **HISTORICAL — describes the earliest version of the project's repo
> structure**, from before the admin panel was consolidated into a
> single `admin.html`, before `assets/js/` existed (this doc still says
> `js/`), and before the `supabase/migration/` (singular) naming and
> numbering used everywhere else in `docs/`. Useful for understanding
> *how* files get pushed to GitHub/Vercel, not useful as a current file
> list — for that, check the "Files changed" section of the most recent
