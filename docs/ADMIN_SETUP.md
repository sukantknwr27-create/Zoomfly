...
> ⚠️ **Note:** the "8 sub-pages" and the `admin-bookings.html` example
> below describe the admin panel *before* `CHANGES_FULL_ADMIN_CONSOLIDATION.md`
> merged everything into one `pages/admin.html` with tabs. Those 8
> standalone files were deleted as part of that round (confirmed — no
> references to them remain anywhere in the codebase). The subdomain
> routing and `requireAdmin()` fixes described here are still accurate
> in spirit — just mentally replace any `admin-*.html` reference with
> "the relevant tab inside `admin.html`."
...
- Visiting `admin.zoomfly.in/pages/admin.html` → works directly (assets/pages pass through untouched on any host; `admin.html` is now the single consolidated admin page — see note at top of this doc)
