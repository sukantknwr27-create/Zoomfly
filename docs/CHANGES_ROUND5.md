...
- Interestingly, this exact bug was already diagnosed and fixed once
  before, in `01_zoomfly_growth_features.sql` (section 18 of
  `CHANGES_ROUND2.md`, "Confirmed root cause of the 403s in the admin
  console") — but `00_zoomfly_master_schema.sql` (the "run fresh on a
  wiped DB" file)
...
- A new **Commission Management** page — added as a new item inside
  the "Travel Partners" sidebar *group* (the group label survives even
  though the redundant standalone "Travel Partners" *page* from #7 was
  removed; Vendors, Agents, and this new page now sit under that same
  group heading) — with:
