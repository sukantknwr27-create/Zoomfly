...
> ⚠️ **Correction (post-audit):** the "0 emoji remain" / "replaced with
> proper inline SVG icons" claims below do **not** match the delivered
> code. A live code audit found 361+ `<span class="emoji-icon">` call
> sites still using raw emoji (including the password show/hide toggle
> named as an example below, which uses 👁️/🙈 emoji, not SVG). Whatever
> shipped in this round wasn't what's described here — either it was
> reverted, or this doc describes work that didn't actually land. The
> current, real state of the codebase is: a consistent, deliberately
> class-wrapped (`emoji-icon`) emoji icon system, used site-wide, and
> it works — it's just not SVG as claimed. Decide separately whether
> that's worth a real SVG pass; don't take the claims below as fact
> until then.
