> ⚠️ **HISTORICAL — describes the earliest version of the project.**
> This guide reflects the very first backend setup: `js/supabase.js`
> (now `assets/js/supabase.js`), one migration file
> `supabase/migrations/001_schema.sql` (now `supabase/migration/`,
> singular, with `00_zoomfly_master_schema.sql` plus migrations
> `01`–`18`), only 4 edge functions and 10 tables (there are many more
> now — see `docs/INDEX.md`'s changelog table for everything added
> since). Keep this for the *general shape* of the Supabase/Razorpay/
> Resend/Vercel setup — but for actual current file paths, migration
> names, table list, and edge function list, use `docs/SESSION_SUMMARY.md`
> and the most recent changelog in `docs/INDEX.md` instead.
